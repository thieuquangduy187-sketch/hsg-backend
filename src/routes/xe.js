...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    function mapRow(row) {
      ...
      const loaiThung = get('Loại Thùng
(Lửng, mui bạt, có cẩu)','Loại Thùng','Loai Thung','LoaiThung')
      const loaiXe    = get('Loại xe','Loai Xe','loaiXe','LoaiXe')
      const taiTrong  = get('Tải trọng 
(Tấn)','Tải trọng','Tai trong','taiTrong')
      const cuaHang   = get('Cưả hàng sử dụng','Cửa hàng','Cua hang','cuaHang')
      const tinhMoi   = get('Tỉnh mới','Tinh moi','tinhMoi','Tỉnh')
...
(Tấn)'] = taiTrong
    ...
    for (const row of rows) {
      try {
        ...
        await col.updateOne(filter, { $set: mapped }, { upsert: true })
        upserted++
      } catch(e) {
