...
router.get('/status', async (req, res) => {
  try {
    ...
    const allXe = await mongoose.connection.db.collection('xetai').find({}, {
    ...
    for (const xe of allXe) {
      ...
      if (bs) {
        ...
        xeInfoMap.set(bs.replace(/[-\.]/g,'').toUpperCase(), info) // "61H07661"
        xeInfoMap.set(bs.toUpperCase(), info)                       // "61H-076.61"
      }
    }
...
router.post('/upload-camera-excel', async (req, res) => {
  try {
    ...
    for (const xe of allXe) {
      ...
      if (bs) {
        const info = { cuaHang: xe['Cưả hàng sử dụng']||'', tinhMoi: xe['Tỉnh mới']||'' }
        xeMap.set(normBS(bs), info)
        xeMap.set(bs.replace(/[-\.]/g,'').toUpperCase(), info)
        xeMap.set(bs.toUpperCase(), info)
      }
    }).toArray()
    const xeMap = new Map()
    for (const xe of allXe) {
